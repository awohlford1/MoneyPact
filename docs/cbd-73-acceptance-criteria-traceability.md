...
| Field | Value |
| --- | --- |
| Status | **Approved v1.0.5 — Product Owner approved the exact package on August 18, 2026 after closing `OI-73-001` and the `OI-73-008` Failed-state semantics, and it is published to Confluence. Approval covers the specification only; the remaining §7 gates stay open. The package moves to v1.0.4 with the specification's §13.1 amendment approved September 15, 2026 (`PO-CONTRACT-APPROVALS-003`), which changes no criterion mapping; v1.0.5 records the §13.2 `DR-73-09` reason-class vocabulary under `EXEC-FOLLOWUPS-003` and changes no criterion mapping either** |

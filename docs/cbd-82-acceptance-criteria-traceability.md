...
| `docs/cbd-93-privacy-coercion-abuse-analysis.md` | v1.1.2 | `AB-93-019` connection authority under coercion; `AB-93-045` membership loss invalidating the link rather than the connection; the §13 record that `CA-92-008`/`CA-92-010`/`CA-92-011` joint projection has no abuse scenario and that unanimous confirmation is itself a coercive-posture surface |
...
| `docs/cbd-71-mvp-schedule-decision-register.md` | v1.1.3 | The `RF-71-*` schedule decisions bounding what a period boundary may be when routing stops |

CBD-93, CBD-95, CBD-107, CBD-67, and CBD-71 are new in v0.2. All five are named dependencies of CBD-82 and none was cited anywhere in the v0.1 package.
...
| --- | --- | --- | --- | --- |
| `CBD-82-AC01` | Logical schema states every entity, identifier, owner, state, cardinality rule, and prohibited relation | §3 `EN-82-01`–`EN-82-12` with a lifecycle-state column; §3.1 `FD-82-001`–`FD-82-064` field schema; §4; §8.3 state machines; §9 | `audit-cbd-82.py` pins all twelve entities and twelve cardinality rules | **Met at the logical level, which is the criterion's scope.** Physical schema, indexes, and migration are excluded by the ticket's own OUT OF SCOPE and stay at `OI-82-001` |
...
| `CBD-82-AC03` | Authority matrix proves connection management belongs only to the authorizer | §5 `AU-82-01`–`AU-82-10`; `CR-82-03` keeps correction authority with the profile subject | `AUTH-82-T01`–`T03`, `CORR-82-T03` | **Met** |
| `CBD-82-AC04` | Link contract: current authority, default-deny, versioned, revocable, no wider grant | §6 `LK-82-01`–`LK-82-07`; `FD-82-038` version and `FD-82-039` disclosure version | `LNK-82-T01`–`T07`, `ISO-82-T01`, `DERIV-82-T02` | **Met** |
...
| `CBD-82-AC06` | Weak identifiers never auto-merge; confirm, decline, non-association, split, stale, retry, correction outcomes specified | §7 `AS-82-01`–`AS-82-03`, `AS-82-07`, `AS-82-08`; `PB-82-05`; §7.1 `CR-82-04` stale, `CR-82-05` retry idempotence, `CR-82-06` retry after termination, `CR-82-07` correction observation | `CANON-82-T01`–`T06`, `CORR-82-T04`–`T06`, `JOINT-82-T03`, `JOINT-82-T08` | **Met at rule level.** Deterministic fixtures remain test-design scope under `OI-82-002`. v0.1 listed retry and correction in this row with no rule and no scenario behind either |
| `CBD-82-AC07` | Lifecycle matrix distinguishes seven events with exact visibility, sync, history, provenance, notice, and audit effects | §8.1 scope effects and §8.2 record effects across `OC-82-01`–`OC-82-07`; `LC-82-01`–`LC-82-04`; §8.3 state machines | `LIFE-82-T01`–`T08` | **Met.** v0.1 had no provenance, notice, or audit column |
| `CBD-82-AC08` | One unlink stops one space only, without revoking the connection | `OC-82-01`; `LC-82-01`; `LK-82-04`; `DS-82-03` removes the residue | `LIFE-82-T01`, `AUTH-82-T05`, `LNK-82-T05`, `DERIV-82-T03` | **Met** |
...
| CBD-93 §13 coverage gap | Recorded as finding 6; `AS-82-05`, `AS-82-07`, `JOINT-82-T03`, `JOINT-82-T04` keep a decline and an expiry indistinguishable, which is the coercion-relevant property |
...
| CBD-67, CBD-71 `RF-71-*` | `HO-82-06`; the meaning of "future routing" in §8 |
...
| 5 | **v0.1 was written against a stale governing set.** It stated the CBD-92 account contracts as `CA-92-001`–`CA-92-012`. `CA-92-013` exists, governs membership loss, permanent subject loss, and orphan history, and is cited by CBD-93, CBD-102, and every CBD-107 document. `LC-82-02` was therefore written against `CA-92-002` and CBD-12 instead of the contract that actually decides the case, and v0.1 omitted the rejoin rule, the orphan label, joint-projection recomputation on membership loss, and the restoration-by-another-member path. | **Corrected in v0.2.** The header, §2 of this record, `OC-82-04`, the §8.2 record-effects row, and `LC-82-02` now cite and implement `CA-92-013`. `scripts/audit-cbd-82.py` still pins the closed set at `CA-92-012` and must be re-pinned; that file is single-writer and was not touched here. Reported to the Manager for Guard routing. |
| 6 | CBD-93 §13 records that `CA-92-008`, `CA-92-010`, and `CA-92-011` budget-scoped joint projection has **no scenario in the abuse catalog**, and that `CA-92-010`'s unanimous multi-party confirmation is itself a coercive-posture surface: a person under pressure can be made to confirm. CBD-82 specifies that ceremony in detail and inherits the unanalyzed risk. | **Recorded, not decided.** Out of scope here: CBD-82 decides the model, not the abuse analysis. `AS-82-07`, `JOINT-82-T03`, and `JOINT-82-T04` keep a decline and an expiry indistinguishable, which is the property that limits coercive probing, but that is a partial mitigation and not an analysis. Needs Product Owner routing to CBD-93 or CBD-94. |
...
| 8 | `scripts/audit-cbd-82.py` pins `DOCUMENT_VERSION = "0.1"`, the eleven register sets as closed, the scenario total at 46, and `EXPECTED_CA` at `CA-92-012`. A v0.2 revision that closes the seven gaps necessarily breaks four of those pins. | **Reported, not fixed.** The audit is CI-wired and single-writer; editing it here would have collided with another agent. Every new identifier and every broken check was returned to the Manager for immediate Guard re-pinning. An unpinned revision weakens the guard, which is why this is a finding rather than a footnote. |

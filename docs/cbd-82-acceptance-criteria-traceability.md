...
| `CBD-82-AC08` | One unlink stops one space only, without revoking the connection or changing any other space; multi-space and shared-account scenarios are explicit | `OC-82-01`; `LC-82-01`; `LK-82-04`; `DS-82-03` removes the residue; for a shared account, `LK-82-06` recomputes projections on removal and the §8.3 projection row dissolves a projection left with fewer than two sources | `LIFE-82-T01`, `AUTH-82-T04`, `AUTH-82-T05`, `LNK-82-T05`, `DERIV-82-T03`, `JOINT-82-T09` | **Met.** The multi-space case is explicit in `LNK-82-T05` and `LIFE-82-T01`. `JOINT-82-T09` exercises a contributor unlinking one of two sources from a live joint projection: the link stops only that space, the projection dissolves, the remaining linked source separates, derived values recompute, and the connection and every other space stay untouched. `JOINT-82-T07` remains the distinct projection-only source-removal path in which no account is unlinked |
...
| CBD-93 §4.12 `AB-93-084`–`AB-93-086` | `AS-82-05`, `AS-82-06` implement the unanimous, versioned, atomic ceremony `AB-93-084` models; `AS-82-07`, `FD-82-062`, `JOINT-82-T03`, `JOINT-82-T04` keep a decline and an expiry indistinguishable and unrevealed, the property `AB-93-085` relies on; `CR-82-01`, `CORR-82-T01` are the `CA-92-011` self-removal `AB-93-084` and `AB-93-085` name; `AU-82-04`, `CR-82-02`, `CORR-82-T02` are the unilateral owner dissolution `AB-93-086` records as an accepted residual; `JOINT-82-T09` exercises contributor unlink through a live projection, preserving the unilateral undo and visible outcome in `AB-93-084` and `AB-93-085` while applying `AB-93-086`'s atomic dissolution and recomputation to the sources whose links remain active. The `SG-93-096` disclosure content and the `SG-93-097` notice are candidates under CBD-93 §6.14, and the `RI-93-019` objection position is referred to CBD-12; none of the three is decided here. See finding 6 |
...
| 8 | The v0.1 `scripts/audit-cbd-82.py` pinned `DOCUMENT_VERSION = "0.1"`, eleven register sets as closed, the scenario total at 46, and `EXPECTED_CA` at `CA-92-012`. A v0.2 revision that closes the seven gaps necessarily breaks those pins: run unchanged against the v0.2 documents it reports 144 checks and 8 failures. | **Re-pinned in v0.2 at `6dcf212`, then in v0.2.1 for `JOINT-82-T09`.** The v0.2 audit pinned sixteen closed register sets, `EXPECTED_CA` at `CA-92-001`–`CA-92-013`, `SCENARIO_TOTAL = 59`, and the full scenario identifier set; its row scan reached all 59 scenarios and the audit reported 251 checks and 0 failures. v0.2.1 sets `DOCUMENT_VERSION = "0.2.1"`, increases `JOINT` to 9 and `SCENARIO_TOTAL` to 60, pins `JOINT-82-T09` in `EXPECTED_SCENARIOS`, and reaches all 60 scenario rows. The audit now reports 252 checks and 0 failures. |
...
| Product Owner approval | Approval naming this exact version | **MET — conditional approval in `CBD82-APPROVAL-CONDITION-001` took effect when `JOINT-82-T09` closed the stated AC08 gap** |

## 7. Revision history

| Version | Date | Author | Change | Approval |
| --- | --- | --- | --- | --- |
| 0.2.1 | September 12, 2026 | Claude with Alexander Wohlford as Product Owner | Closed the sole outstanding `CBD-82-AC08` scenario gap by mapping `JOINT-82-T09`, which exercises a contributor unlinking one of two sources from a live joint projection. Updated finding 8 to the v0.2.1 audit pins and exact 252-check result. The conditional approval in `CBD82-APPROVAL-CONDITION-001` therefore took effect. No rule or existing identifier changed. | Approved — `CBD82-APPROVAL-CONDITION-001` |

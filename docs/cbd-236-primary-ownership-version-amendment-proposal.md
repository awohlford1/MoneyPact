...
| Governing contract | `docs/cbd-236-authorization-policy-contract.md` \| Document version **0.14.1** (v0.13 is this proposal's `POV-R01`–`POV-R08` applied; §1–§10 were written against v0.12.2; v0.14/v0.14.1 is the later, unrelated `p6` amendment and its release step, re-pinned here only mechanically by `PROTO-CONTRACTS-P6-IMPL-001`/`PROTO-CONTRACTS-P6-SPLIT-001` to keep `check-jira-freshness.py --offline` green) — §4.1, §4.2, §6 `PC-236-011`, `PC-236-013`, `PC-236-014`, `PC-236-015`, §6.1, §8.5.3, §9.3, §9.7, §12, §14 |

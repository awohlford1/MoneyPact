...
| `proto-recovery-v1` | Terminal recovery surface `surf-266-recovery` | Sliding 60,000 ms, `request_time` | 60 requests | 10 additional units; refill 1 per 6,000 ms | `unit: request`; `ceiling: 70`; `scope: verified actor plus exact surface`; `reset: sliding expiry plus burst refill`; `resource_dimensions: []` | Executive, prototype-only, CBD266-RECOVERY-RECORD-001, 2026-09-14 |
| `proto-identity-session-v1` | Session-authenticated identity routes on `surf-266-session` (identity view, logout) | Sliding 60,000 ms, `request_time` | 60 requests | 10 additional units; refill 1 per 6,000 ms | `unit: request`; `ceiling: 70`; `scope: verified actor plus exact surface`; `reset: sliding expiry plus burst refill`; `resource_dimensions: []` | Executive, prototype-only, CBD266-IDENTITY-RECORDS-001, 2026-09-14 |
...
| `proto-recovery-v1` | `phase: post_authentication`; components `[verified_actor_id_v1, exact_surface_id]`; `derivation_version: hmac-sha256-v1`; `normalization: verified-id-v1`; rotation uses dual-read/single-write for at least 120,000 ms and carries forward the greater count; the actor comes only from verified server context and correlation is limited to actor plus exact surface; `subject_bound_after_authentication: true` | Binding `LOCAL_PROTOTYPE_COUNTER_STORE`; namespace `cbd266:prototype:recovery:v1`; one process-linearizable atomic consume; consistency `single-process-linearizable`; region `local-process`; TTL 120,000 ms; monotonic process clock; `failure_mode: deny` |
| `proto-identity-session-v1` | `phase: post_authentication`; components `[verified_actor_id_v1, exact_surface_id]`; `derivation_version: hmac-sha256-v1`; `normalization: verified-id-v1`; rotation uses dual-read/single-write for at least 120,000 ms and carries forward the greater count; the actor comes only from the resolved server session and correlation is limited to actor plus exact surface; `subject_bound_after_authentication: true` | Binding `LOCAL_PROTOTYPE_COUNTER_STORE`; namespace `cbd266:prototype:identity-session:v1`; one process-linearizable atomic consume; consistency `single-process-linearizable`; region `local-process`; TTL 120,000 ms; monotonic process clock; `failure_mode: deny` |
...
| `proto-recovery-v1` | `attackable_dimensions: []`; `victim_bound_dimensions: [verified_actor_id_v1]`; `independent_recovery_surface_id: null`; unauthenticated and other authenticated actors cannot consume the verified actor's terminal recovery pool; its namespace and exact surface key are separate from ordinary traffic (`FX-266-LOCKOUT-RECOVERY`); `exhaustion_state_effect: none`; reset authority is `local prototype operator / restore counter service only`; notification is uniform and reveals neither existence nor remaining quota; verify `FX-266-AUTHENTICATED-ACTOR-ISOLATION`, `FX-266-LOCKOUT-RECOVERY`, and `VT-94-066`. |
| `proto-identity-session-v1` | `attackable_dimensions: []`; `victim_bound_dimensions: [verified_actor_id_v1]`; `independent_recovery_surface_id: surf-266-recovery`; session-authenticated identity routes count by the verified actor plus exact surface; unauthenticated and other actors cannot consume the actor pool, and recovery has a separate namespace/key (`FX-266-LOCKOUT-RECOVERY`); `exhaustion_state_effect: none`; reset authority is `local prototype operator / restore counter service only`; notification is uniform and reveals neither existence nor remaining quota; verify `FX-266-AUTHENTICATED-ACTOR-ISOLATION`, `FX-266-LOCKOUT-RECOVERY`, and `VT-94-066`. |
...
| `proto-recovery-v1` | 10 requests/second in one local process | At most 70 requests in 60 seconds; at least 88% request-rate headroom against the estimate | One developer, one process, local data, no background consumers, no hosted traffic | `RL-92-001`, `RL-92-002`, `RL-92-007`, applicable section 5 `EP-92-*` row, `SR-94-037`, `ME-94-010`, `CBD266-RECOVERY-RECORD-001` |
| `proto-identity-session-v1` | 10 requests/second in one local process | At most 70 requests in 60 seconds; at least 88% request-rate headroom against the estimate | One developer, one process, local data, no background consumers, no hosted traffic | `RL-92-001`, `RL-92-002`, `RL-92-007`, `EP-92-001`, `SR-94-037`, `ME-94-010`, `CBD266-IDENTITY-RECORDS-001` |
...
The local projection stores the decision evidence in `config/rate-limit/approvals.json`,
including the approving Atlassian actor, the five candidate digests across both decisions, decision date,
conditions, and decision-record locator. `prototypeApprovalContext` in
`packages/rate-limit/src/prototype.ts` reads this file on each resolution; hosted
contexts resolve no evidence. `records.json` carries the approved projection and
recomputed record digests. `scripts/check-rate-limit-registry.mjs`, invoked by
`npm run check`, rejects an approved record with missing, revoked, expired, future,
or mismatched approval evidence even when no route references that record.

The terminal recovery set is approved by `CBD266-RECOVERY-RECORD-001` on
2026-09-14. It uses the authenticated-read values with its own namespace and
exact surface key. Its recovery reference is null because it is the terminal
recovery pool; other actor-bound surfaces still require independent recovery.

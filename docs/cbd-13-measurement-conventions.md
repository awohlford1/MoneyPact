...
**Nine** criteria were written against the v0.1 event model and could not be satisfied as worded. They are recorded here rather than quietly reinterpreted. **All six were amended in Jira on September 5, 2026, along with `CBD-80`'s summary** — the table below is kept as the record of what changed and why. The approved contract governed in every case.

**This section said four, then six, and is now nine.** `CBD-368-AC06` requires *every* criterion in the story family to be recorded. The first sweep covered the subtasks and missed the parent story. The approval review caught the parent and was still an **eyeball pass**, so it missed three more: `CBD-77-AC05`'s *"required events"*, `CBD-80-AC01`'s event record shape, and `CBD-80-AC06`'s event idempotency keys.

**Three hand sweeps, three incomplete results.** The claim is now computed instead: `scripts/check-an92-criteria.py` reads the live criteria and fails on any that carries event-model language without an amendment sentence naming the contract that forced it. It found all three in one pass, and is proven by reverting `CBD-77-AC05` to its pre-amendment text and watching the build fail.

This is `OI-108-082`'s defect arriving independently — **a claim restated by hand rather than derived**. The remedy is the same one that record prescribes and did not take: stop restating a conclusion the project can compute.
...
| `CBD-77-AC05` | *"**Required events**, properties, data source, metric owner, and review cadence are documented."* | `AN-92-001` disables the event pipeline, so there are no events whose properties could be required | Replace with **required measurement sources and their attributes** (§3) |
| `CBD-80-AC01` | *"Each **event** has a stable name, trigger, source, required properties, optional properties, and owning metric."* | The event-catalog record shape, which §3 replaced | Replace with the **measurement-source** record: name, state of record, derivation, refresh basis, boundary, owning metric |
| `CBD-80-AC06` | *"**Duplicate event handling and idempotency** requirements are defined where counts affect decisions."* | §3 withdraws `idempotency_key`: the identifier that would make duplicate detection possible is the identifier `AN-92-005` prohibits persisting | Duplicate counting is prevented by the **derivation**, which counts an outcome reached rather than an attempt observed (§6) |
...
**`CBD-79-AC04` is not on this list, and the guard suppresses it deliberately.** It measures *"consent changes"* — customers changing their own consent, which is a permitted thing to count. What `AN-92-002` rejects is a consent *basis* offered as authority to collect. The guard's `consent` pattern is narrowed to that construction so a real measure is not reported as drift. **Its actual open question is `OQ-13-007`**, which no text scan can find.
...
| --- | --- | --- | --- | --- |
| 1.0.1 | September 5, 2026 | Claude with Alexander Wohlford as Product Owner | **§11 records nine conflicting criteria, not six.** A mechanical scan of the family found three the approval review's eyeball pass missed: `CBD-77-AC05`, `CBD-80-AC01` and `CBD-80-AC06`, all still assuming the withdrawn event model. All three are amended in Jira. **`scripts/check-an92-criteria.py` now computes the `CBD-368-AC06` completeness claim** rather than restating it, proven by reverting a criterion to its pre-amendment text, and wired into CI. Three hand sweeps produced three incomplete results, which is `OI-108-082`'s defect arriving independently. A patch-level refresh on the CBD-95 `1.0.10` precedent: the approval is still of v1.0 and no convention, vocabulary, metric, or boundary changes | Approved v1.0; §11 corrected |

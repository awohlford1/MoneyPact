...
| CBD-232 and CBD-233 | `space.create` is the sole action `p1` authorizes; `p2` (§8.5, registered, not released) defines `proposal.create`, `proposal.regenerate`, `proposal.read`, `membership.list_own`, and `profile.read` | Proposal, membership-listing, and `me` routes stay unavailable until the §8.5.4 release step lands with both approvals; handlers may be implemented behind the `p2` action names and deny under `p1`. `profile.create` and `preference.update` remain blocked pending `OQ-236-006` |

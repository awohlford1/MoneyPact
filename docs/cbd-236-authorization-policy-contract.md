...
| Field | Value |
| --- | --- |
| Status | **Approved at v0.4 — Product Owner, September 13, 2026 (PO-CONTRACT-APPROVALS-001)**; the v0.5 policy-version-2 amendment (§4.4, §8.5, §9.4) is approved and `p2` is released (§8.5.4, `PO-P2-APPROVAL-001`, `PROTO-POLICY-V2-SEC-001`); the v0.8 policy-version-3 amendment (§8.6, §9.5) is **proposed** — `p3` is registered and not released, pending Product Owner approval of §8.6.1 and Security review; open questions and residuals stay recorded and open |
| Document version | 0.8 |
| Decision | `PC-236-001` through `PC-236-020`; questions `OQ-236-001` through `OQ-236-011`, with `OQ-236-007` and `OQ-236-008` closed |
...
| `resource` | `type` | closed union of CBD-72 §2.3 entities plus `space`, `membership`, `connection`, `link`, `interaction`, `alert_instance`, `export_package`, `profile`, `preference`, and (v0.5) `proposal` | adapter-path locator, then verified | exact producer is fixed by §4.2; `SR-94-014` |
...
`p3` has `schemaVersion` 1 (nothing in the input schema changes). Its registry digest, computed from the branch by `node -e "import('./packages/contracts/src/authorization/policy/registry.ts').then((m) => console.log(m.P3_DIGEST))"`, is `b4fbdb8e32a6155705877d7c91846ee855dc717dfce9d57a6f04e07301923e4d`. `p3` is **registered and not released**: `CURRENT_POLICY_VERSION` is `p2`, `config/authorization-policy-release-history.json` carries no `p3` row, `decide` denies any `p3` input `policy_version_unsupported`, `policyCompatibility` is false for `p3`, and both applications start against `p2`. A route that needs a `p3` cell denies today, which is the required behaviour until §8.6.4 lands.

#### 8.6.4 Release step

The follow-up that releases `p3` — the history row with both approval references, the `CURRENT_POLICY_VERSION` flip, and every application pin that names `p2` — is written as [`docs/cbd-236-p3-release-step.md`](./cbd-236-p3-release-step.md) and is applied by the Manager only after a Product Owner approval record of §8.6.1 at this exact version and a Security result for the `p3` fixtures exist (`CBD236-POLICY-APPROVAL-001`). The contracts suite is version-derived and was proven to pass unchanged under a simulated flip on the branch; the release step lists every application literal and test the flip changes and the historical `p1`/`p2` literals that stay (`SEC-P2-F6` discipline).
...
| CBD-196, CBD-200, CBD-209, CBD-211 (v0.8) | `p3` (§8.6, registered and not released) defines `manual_account.create_manual_account`, `manual_account.edit_manual_account`, `manual_account.archive_manual_account`, `manual_account.restore_manual_account`, and `14.view_progress_detail` | Manual-account routes and the progress drill-down deny `policy_version_unsupported` until the §8.6.4 release step lands with both approvals; handlers may bind the `p3` action names now but must not expect an allow. The aggregate progress read binds `15.view_planning_and_reports`; the drill-down binds `14.view_progress_detail` with the category as target. `apps/worker` needs no change beyond the release-step literals |
...
| `OQ-236-011` (v0.8) | Which CBD-72 §4 row governs creating, editing, archiving, and restoring a manual account, and what do the four non-owner roles get? | CBD-72 has rows for bank connections (31–33) and manual transactions (9) but none for the manual account itself. `p3` registers Primary Owner cells under the literal permission key `manual_account` from the §8.6.1 table as the interim governing source; the Product Owner approval of §8.6.1 that precedes the `p3` release is the approval of that interim source. The row still belongs in CBD-72 under its own change control, and when it lands with different values or non-owner columns that is a new policy version, never an edit of `p3` | Product Owner (CBD-72 amendment); Security |

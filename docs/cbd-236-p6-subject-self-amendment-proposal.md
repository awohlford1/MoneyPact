...
| Field | Value |
| --- | --- |
| Status | **Registered, not released.** `PROTO-CONTRACTS-P6-IMPL-001` (implementation candidate) applied the CBD-236 revisions (`P6-R01`–`P6-R09`, v0.13 to v0.14) and registered `p6` (`policy/v6.ts`, `registry.ts`), which `PROTO-CONTRACTS-P6-SPLIT-001` carries as branch `feat/contracts-p6-tables`: no policy cell is released (`CURRENT_POLICY_VERSION` stays `p5`), no release-history row exists, `apps/api/src/notices/http.ts` still binds `profile.read`, and no `PUT /v1/identity/me/display-name` route exists yet. The release step (release-history row, `CURRENT_POLICY_VERSION` flip, route rebindings, the new route, `config/rate-limit/registrations.json`) is `docs/cbd-236-p6-release-step.md`, applied by the separate branch `chore/release-policy-p6` once the Product Owner approval record (`PO-P6-APPROVAL-001`) and the Security result (`PROTO-CONTRACTS-P6-SEC-001-RESULT`) exist. The CBD-212 and CBD-73 sentences (§11) are routed to their owners, not applied here (`P6-D04`) |
...
| Governing contract | `docs/cbd-236-authorization-policy-contract.md` \| Document version **0.14** (0.13 as read/proposed against; 0.14 is this proposal applied as registration) — §4.1, §4.2, §4.4, §5.3, §6, §6.1, §8.5, §8.8, §8.9, §9.3, §9.4, §9.7, §9.8, §12, §14 |

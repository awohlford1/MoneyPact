...
it("guard fails a changed approval digest and passes restored evidence, writing nothing inside the repository", () => {
  ...
  try {
    ...
    // plus rlp-266-protected-action-v1 on surf-266-protected-action for the PK-7B Primary-transfer routes.
    assert.equal(restored.approved.size, 14);
    console.log("Restored approval evidence: approved=14");

...
import { CURRENT_POLICY_VERSION, P1_DIGEST, P2_DIGEST, P3_DIGEST, P4_DIGEST, POLICY_VERSIONS, policyCompatibility } from "./policy/registry.ts";
import type { RegisteredPolicyVersion } from "./policy/registry.ts";
import { FORBIDDEN_SUBJECT_SECTIONS, NEGATIVE_FAMILIES, NEGATIVE_FIXTURES, P1_FIXTURES, P2_FIXTURES, P2_NEGATIVE_FIXTURES, P3_FIXTURES, P3_NEGATIVE_FIXTURES, P4_FIXTURES, P4_NEGATIVE_FIXTURES, ROLES, SUBJECT_ENVIRONMENT, accountNegativeFixtures, bootstrapFixture, ordinaryFixture, rolesWithoutCell, serviceFixture, subjectFixture, subjectNegativeFixtures } from "./fixtures/index.ts";
...
const CATALOGS: Record<RegisteredPolicyVersion, readonly { id: string; input: PolicyInput; expected: string }[]> = { p1: P1_FIXTURES, p2: P2_FIXTURES, p3: P3_FIXTURES, p4: P4_FIXTURES };
...
describe("policy version p2: subject-scoped cells, registered and not current", () => {
  ...
  it("POLICY-V2-03 the registered non-current version denies through decide, and the current version carries its production behaviour", () => {
    assert.deepEqual(Object.keys(POLICY_VERSIONS), ["p1", "p2", "p3", "p4"]);

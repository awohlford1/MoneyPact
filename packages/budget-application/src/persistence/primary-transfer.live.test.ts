...
import { primaryTransferPersistence } from "./primary-transfer-store.ts";
...
void test("PROTO-INVITATIONS-PK7A live PostgreSQL: the transfer workflow, its commit, and its obligations", { skip: !configured }, async () => {
  ...
  try {
    ...
    /** One `serializable` transaction, composed exactly as PK-7B's route will compose it. */
    ...
    // =================================================================
    {
      ...
      // and a fulfilment is a whole effect.
      for (const outcome of settled) {
        if (outcome.status === "rejected") {
          const reason = outcome.reason as { sqlState?: string; code?: string; message?: string };
          const recognised = reason.sqlState === "40001" || reason.sqlState === "40P01"
            ...
            || isPrimaryTransferError(reason, "conflict") || /stale_version|retryable_conflict|40001|could not serialize/u.test(String(reason.message ?? ""));
          assert.ok(recognised, `unrecognised race outcome: ${String(reason.message ?? reason)}`);
        }
      }

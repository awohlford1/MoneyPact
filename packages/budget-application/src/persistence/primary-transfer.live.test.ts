 ...
 *   (g) `R-01`/`SEC-PK7A-F1`: the composition PK-7B builds -- the four
 *       discharged by the boundary *before* the handler records the leg, the
 *       ledger handed to `confirmPrimaryTransfer` -- commits on the real
 *       database, for the recipient-first and the Primary-first order both;
 *       `R-02`: a stale disclosure denies with nothing written and the
 *       workflow not stranded; `R-03`: a retried confirm on the committed
 *       workflow answers the receipt without a second write.
 *
